...
export const chainhash = {
  ...
  async hash(hexData: string): Promise<Uint8Array> {
    await init();
    return unwrap<Uint8Array>(g().chainhash.hash(hexData));
  },
  /** Compute double-SHA-256 (SHA256d) of data (hex). Returns hex.
   *  Calls Go: chainhash.DoubleHashB() from btcd/chaincfg/chainhash. */
  async doubleHash(hexData: string): Promise<Uint8Array> {
    await init();
    return unwrap<Uint8Array>(g().chainhash.doubleHash(hexData));
  },
  ...
  async taggedHash(hexTag: string, hexMsgs: string[]): Promise<Uint8Array> {
    await init();
    return unwrap<Uint8Array>(g().chainhash.taggedHash(hexTag, hexMsgs));
  },
